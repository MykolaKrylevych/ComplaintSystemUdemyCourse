...
class SESService:
    def __init__(self):
        ...
        self.ses = boto3.client("ses", region_name=config("SES_REGION"), aws_access_key_id=self.key,
                                aws_secret_access_key=self.secret)

    def send_mail(self, subject, to_addresses, text_data):
        body = {"Text": {"Data": text_data, "Charset": "UTF-8"}}
        self.ses.send_email(Source=config("MY_EMAIL"), Destination={"ToAddresses": to_addresses,
                                                                    "CcAddresses": [],
                                                                    "BccAddresses": []},
